...
export default function ReportsPage() {
  ...
  const incomeTotal = movements.filter(m => m.type === "INCOME").reduce((acc, m) => acc + m.amount, 0);
  const expenseTotal = movements.filter(m => m.type === "EXPENSE").reduce((acc, m) => acc + m.amount, 0);
  ...
  return (
    <DashboardLayout role={userRole}>
      <div className="max-w-5xl mx-auto py-6 px-4">
        <h1 className="text-3xl font-black mb-8 text-gray-900">Análisis Financiero 📊</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          <div className="lg:col-span-1 bg-indigo-600 rounded-[2.5rem] p-10 text-white shadow-xl flex flex-col justify-center min-h-[200px]">
            <p className="text-indigo-200 text-xs font-bold uppercase tracking-widest mb-2">Saldo Neto Actual</p>
            <h2 className="text-4xl font-black">${totalBalance.toLocaleString()}</h2>
          </div>

          <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm min-h-[350px]">
            <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-6 text-center">Comparativa de Flujo de Caja</p>
        ...
        <div className="bg-white p-12 rounded-[2.5rem] border border-gray-100 shadow-sm text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Exportar Movimientos</h3>
          <p className="text-gray-400 mb-8 max-w-sm mx-auto">Genera un archivo CSV con el histórico completo para tu contabilidad en Excel.</p>
          <button 
            onClick={exportCSV}
